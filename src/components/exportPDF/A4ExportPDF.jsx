          ...
          <thead className="bg-gray-800 text-white">
            <tr>
              {Object.entries(data.length >= 1 && data[0]).map(([key_, value], key) => {
                return (
                  <th key={key} className="text-left py-1 px-1 uppercase font-semibold">
                    {key_}
                  </th>
                );
              })}
              ...
              return (
                <tr key={key}>
                  {Object.entries(tr).map(([key_, value], key) => {
                    return <td key={key} className="text-left py-1 px-1">{value.includes(".000Z") ? value.slice(0,10) : value}</td>;
                  })}
                </tr>
              );
            })}
